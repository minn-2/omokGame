...
class Rules:
    ...
    # - 한 좌표를 기준으로 4가지 방향에 대해 양방향(sign=1, -1) 탐색을 수행하므로,
    #   실제로는 8방향 전방위 탐색을 모두 커버
  ...
  @staticmethod
    ...
    def _count_pattern_matches(board, r, c, player, patterns):
        ...
        for dr, dc in Rules.DIRECTIONS:
            # 착수 지점을 중심으로 앞뒤 6칸씩(총 13칸짜리 범위)의 수평선 추출
            line, positions = Rules._get_line_with_pos(board, r, c, dr, dc, board_size, radius=6)

            # 비교하고자 하는 패턴 목록(OPEN_THREE 혹은 OPEN_FOUR)을 하나씩 대조
            for pat in patterns:
                plen = len(pat)
                ...
                for start in range(len(line) - plen + 1):
                    ...
                    # 오목판에서 자른 조각(chunk)이 타겟 패턴과 일치하는 경우
                    if chunk == pat:
